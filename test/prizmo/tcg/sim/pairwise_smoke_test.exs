defmodule Prizmo.Tcg.Sim.PairwiseSmokeTest do
  ...
  test "known deck registry coverage has no unsupported cards" do
    summary = RegistryCoverage.report().summary

    assert summary.behavior_statuses == %{implemented: summary.card_count}
  ...
  for {deck_a, deck_b} <- for(a <- @known_decks, b <- @known_decks, a != b, do: {a, b}) do
    ...
    test "#{deck_a.name()} #{deck_a.id()} can open against #{deck_b.name()} #{deck_b.id()}" do
