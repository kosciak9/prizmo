defmodule Prizmo.TcgEngine.Mechanics do
  ...
  defp resolve_prompt_choice(
         %Game{} = game,
         %Prompt{} = prompt,
         %PendingEffect{source_type: :tool_effect} = pending_effect,
         player_id,
         choice_key,
         normalized_choice
       ) do
    with {:ok, prompt} <- resolve_prompt(prompt, normalized_choice),
         ...
         {:ok, game} <-
           ToolEffects.resume_pending_effect(
             game,
             prompt,
             pending_effect,
             player_id,
  ...
  @spec use_genesect_ex_metallic_signal(Game.t() | String.t(), String.t(), String.t()) ::
          {:ok, Game.t()} | {:error, term()}
  def use_genesect_ex_metallic_signal(game_or_id, player_id, source_card_instance_id)
      when is_binary(player_id) and is_binary(source_card_instance_id) do
    transaction(fn ->
      with {:ok, game} <- get_game(game_or_id),
           {:ok, turn} <- require_action_window_for_player(game, player_id),
           ...
           {:ok, source_card} <- get_card(game.id, source_card_instance_id),
           :ok <- require_card_owned_by_player(source_card, player_id),
           :ok <- AbilityEffects.require_metallic_signal_available(source_card, turn),
           {:ok, deck_cards} <- cards_in_zone(game.id, player_id, :deck),
           legal_choice_cards = AbilityEffects.metallic_signal_legal_choice_cards(deck_cards),
           legal_choice_ids = Enum.map(legal_choice_cards, & &1.id),
           {:ok, current_source_card} <- get_card(game.id, source_card.id),
           {:ok, marked_source_card} <-
             update(current_source_card, :set_markers, %{
               markers: AbilityEffects.put_metallic_signal_used_marker(current_source_card, turn)
             }),
           {:ok, pending_effect} <-
             create(PendingEffect, :create, %{
               game_id: game.id,
               ...
               controller_player_id: player_id,
               current_player_id: player_id,
               effect_key: AbilityEffects.metallic_signal_ability_id(),
               step: "awaiting_choice",
               state: %{
                 "version" => 1,
                 "kind" => "ability_effect",
                 "effect_type" => Atom.to_string(AbilityEffects.metallic_signal_ability_id()),
                 "player_id" => player_id,
                 "source_card_instance_id" => marked_source_card.id,
                 "source_card_id" => marked_source_card.card_id,
                 "legal_choice_ids" => legal_choice_ids
           ...
           {:ok, _prompt} <-
             create(Prompt, :create, %{
               ...
               player_id: player_id,
               payload: %{
                 "choice_key" => Atom.to_string(AbilityEffects.metallic_signal_ability_id()),
                 "legal_choices" => legal_choice_ids,
                 "legal_choice_labels" =>
                   AbilityEffects.metallic_signal_choice_labels(legal_choice_cards),
                 "min" => 0,
                 "max" => AbilityEffects.metallic_signal_max_targets(),
                 "source_card_instance_id" => marked_source_card.id,
                 "source_card_id" => marked_source_card.card_id
               }
             }),
           {:ok, _event} <-
             write_event_and_snapshot(
               ...
               :ability_used,
               player_id,
               metallic_signal_event_payload(turn, marked_source_card, legal_choice_cards)
             ) do
        get_game(game.id)
      end
    end)
  ...
  defp metallic_signal_event_payload(
         %Turn{} = turn,
         %CardInstance{} = source_card,
         legal_choice_cards
       )
       when is_list(legal_choice_cards) do
    %{
      "turn_id" => turn.id,
      "turn_number" => turn.turn_number,
      "player_id" => source_card.owner_player_id,
      "ability_id" => Atom.to_string(AbilityEffects.metallic_signal_ability_id()),
      "source_card_id" => source_card.card_id,
      "source_card_instance_id" => source_card.id,
      "legal_choice_count" => length(legal_choice_cards),
      "public_note" => "Genesect ex used Metallic Signal."
    }
  end
