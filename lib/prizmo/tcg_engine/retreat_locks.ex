defmodule Prizmo.TcgEngine.RetreatLocks do
  @moduledoc false

  alias Prizmo.TcgEngine.CardInstance
  alias Prizmo.TcgEngine.Turn

  @cannot_retreat_next_turn_key "cannot_retreat_next_turn"

  @spec put_cannot_retreat_next_turn_marker(CardInstance.t(), Turn.t()) :: map()
  def put_cannot_retreat_next_turn_marker(%CardInstance{markers: markers}, %Turn{} = turn) do
    markers
    |> normalize_markers()
    |> Map.put(@cannot_retreat_next_turn_key, cannot_retreat_next_turn_marker(turn))
  end

  @spec cannot_retreat_next_turn_marker(Turn.t()) :: map()
  def cannot_retreat_next_turn_marker(%Turn{
  ...
  def blocked_this_turn?(%CardInstance{markers: markers}, %Turn{turn_number: turn_number})
      when is_integer(turn_number) do
    markers
    |> persisted_cannot_retreat_next_turn_marker()
  ...
  defp persisted_cannot_retreat_next_turn_marker(markers) when is_map(markers) do
    Map.get(markers, @cannot_retreat_next_turn_key) || Map.get(markers, :cannot_retreat_next_turn)
  end

  defp persisted_cannot_retreat_next_turn_marker(_markers), do: nil
