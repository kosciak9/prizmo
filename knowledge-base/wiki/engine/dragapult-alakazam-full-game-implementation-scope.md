...
- `SCR-118` Fan Rotom is the current newly surfaced Goal 1 blocker from broader validation. Implement `Fan Call` as a first-turn, once-per-turn Ability command that searches up to 3 Colorless Pokémon with 100 HP or less from deck to hand, shuffles, emits viewer-safe events, opens/resolves through a prompt when choices are needed, and appears in the React play surface.
