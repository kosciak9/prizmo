...
- Broader/deeper seeded latest-result browser validation now also includes live Dragapult `27611` versus live Alakazam `28368` in supported game `1d559bbd-2529-448a-a0ea-eb583a364890` with explicit seed `goal1-27611-vs-28368-depth-608`. That run completed two-seat setup with `Moltres` + `Munkidori` versus `Genesect`, used player-1 Turn 1 to establish `MEG-127` Risky Ruins, then exercised a player-2 Turn 2 chained mid-game line entirely through the browser: `PFL-087` Dawn opened the full staged Basic/Stage 1/Stage 2 search prompt, resolved `Abra` + `Kadabra` + `Alakazam`, and returned to the action window; benching the searched `Abra` from hand under the opposing `Risky Ruins` applied 20 damage to that Abra; attaching `POR-088` Telepathic Psychic Energy to the damaged Abra opened the attach-trigger prompt with four legal Basic Psychic deck choices; resolving that prompt with mixed `Abra` + `Dedenne` choices benched both Pokémon from deck, shuffled, and returned to the action window; and player 2 then replaced the Stadium with `PFL-085` Battle Cage before passing. No visible `Pending card text` blockers or browser console errors surfaced. Tidewave verification confirmed the post-pass state on player-1 Turn 3 with `PFL-085` active, player-2 Bench count `3`, the original benched `Abra` carrying `20` damage plus attached `POR-088`, and the extra `Abra` + `Dedenne` on Bench from the attach-trigger resolution.
...
- Remaining live validation work is broader and deeper: continue two-seat play-surface runs across more of the latest-result Goal 1 fixture pool and keep pushing beyond setup/pass into additional mid-game turn lines when those runs provide new coverage signal. The fixture launcher now supports explicit RNG seeds, and `mix prizmo.goal1.seed_search` can now preselect promising fixture seeds before opening the browser. Prefer regression-fixture mode when reproducing seed-search hits, because regular premade/open-deck creation uses different synthetic deck-key shuffle context. The `28264` / `28405` `TWM-082` line and the `27611` / `28368` Dawn + Risky Ruins + `POR-088` + Battle Cage line are now browser-validated through meaningful mid-game follow-up, so prefer the remaining unproven latest-result ids (`28259` on Dragapult and `28385`, `28398`, `27615` on Alakazam) or other unexplored seeds that exercise new prompt-heavy or passive-effect states.
