...
- Extend the committed Goal 1 fixture corpus to cover the current live latest-result deck ids for Dragapult, or explicitly freeze a narrower representative fixture set with rationale. Use `mix prizmo.goal1.corpus` as the source of truth for that decision instead of relying on the historical `27431` fixture set by default.
- Current live latest-result cards for both Dragapult and Alakazam are fully supported on the canonical Ash path; remaining Goal 1 live-card gaps are now fixture-drift and full-play validation work rather than card-definition gaps.
