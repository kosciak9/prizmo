...
- The latest Alakazam trainer-support batch closed three of those live blockers on the canonical path: `ASC-197` Nighttime Mine now adds `{C}` to attacks used by each Tera Pokémon in play, `TEF-146` Eri now resolves through `play_card` with an acting-player prompt over the opponent's revealed Item cards, and `TEF-159` Rescue Board now reduces Retreat Cost by `{C}` or to zero at 30 remaining HP or less. After that batch, `mix prizmo.goal1.corpus` reports Alakazam live coverage at `supported=32`, `generic-supported=1`, `partial=2`, `unimplemented=4`; the remaining latest-result non-full-support cards are `MEG-130` Switch (metadata missing), `PFL-094` Wondrous Patch, `TWM-082` Alakazam, `TWM-141` Bloodmoon Ursaluna ex, `BLK-040` Elgyem, and the pre-existing `POR-088` Telepathic Psychic Energy partial.
...
- Alakazam live-corpus support slice: close `PFL-094`, `TWM-082`, `TWM-141`, `BLK-040`, and the missing `MEG-130` metadata; `POR-088` remains the broadest pre-existing partial across the full current Alakazam latest-result field.
