...
- The latest Alakazam support batch closed three more live blockers on the canonical path: `MEG-130` Switch now has committed metadata plus an engine-defined `play_card` prompt over the acting player's Bench, `PFL-094` Wondrous Patch now resolves through a mixed discard-Energy/Bench-Psychic prompt, and `TWM-141` Bloodmoon Ursaluna ex now has executable `Seasoned Skill` prize-based attack-cost reduction plus `Blood Moon` attack-lock resolution. That batch also corrected `AttackCosts` to read real Prize-zone counts for prize-based reductions instead of a nonexistent player field. After that batch, `mix prizmo.goal1.corpus` reports Alakazam live coverage at `supported=35`, `generic-supported=1`, `partial=2`, `unimplemented=1`; the remaining latest-result non-full-support cards are `TWM-082` Alakazam, `BLK-040` Elgyem, and the pre-existing `POR-088` Telepathic Psychic Energy partial.
...
- Alakazam live-corpus support slice: close `TWM-082` Alakazam and `BLK-040` Elgyem on the canonical path; `POR-088` remains the broadest pre-existing partial across the full current Alakazam latest-result field.
