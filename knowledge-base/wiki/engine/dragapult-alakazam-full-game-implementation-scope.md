...
- Remaining live validation work is broader and deeper: continue two-seat play-surface runs across more of the latest-result Goal 1 fixture pool and push beyond setup/pass into mid-game turn lines when those runs provide new coverage signal. The fixture launcher now supports explicit RNG seeds, so use seeded runs when reproducing setup or mid-game issues across both seats.
