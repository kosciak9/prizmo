...
- The latest Alakazam support follow-up fully closed `TWM-082` Alakazam on the canonical path. `BLK-040` Elgyem now has an executable `Slight Shift` declared-attack resolution path that moves an opponent's attached Energy to another opponent Pokémon through the Ash attack-resolution surface plus SPA controls, and `TWM-082` Alakazam now has both its executable `Psychic` attack (`10 + 50` per Energy attached to the opponent's Active) and its full `Strange Hacking` attack surface. `Strange Hacking` now Confuses the opponent's Active Pokémon and supports optional post-damage counter redistribution across the opponent's in-play Pokémon through the Ash attack-resolution command, KO/prize plumbing, flow-state handoff, and SPA attack-resolution controls. After this batch, `mix prizmo.goal1.corpus` reports Alakazam live coverage at `supported=37`, `generic-supported=1`, `partial=1`, `unimplemented=0`; the only remaining latest-result non-full-support card is the pre-existing `POR-088` Telepathic Psychic Energy partial.
...
- Alakazam live-corpus follow-up: `POR-088` Telepathic Psychic Energy remains the only current latest-result Alakazam partial and the broadest pre-existing unresolved special-energy behavior across the full current Alakazam field.
