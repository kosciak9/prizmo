...
The latest Goal 2 engine batch closed the two highest-share shared Supporter blockers from the three-archetype queue before dropping into harder Pokémon ex and Special Energy work. `TEF-155` Morty's Conviction now has committed TCGdex metadata plus a real discard-1-cost Supporter path that draws `1` card for each of the opponent's Benched Pokémon. `TWM-145` Carmine now has committed TCGdex metadata plus a real first-turn-legal Supporter path that discards the player's remaining hand and draws `5`. The reusable addition is trainer-side dynamic draw-count handling alongside partial-draw discard-hand-then-draw resolution, which should help later non-search Supporters without reopening Goal 1.

### Highest-priority remaining `unimplemented` cards from current report

| Card | Archetypes | Total share | Notes |
| --- | --- | --- | --- |
| `TWM-112` Cornerstone Mask Ogerpon ex | `3` | `15.62%` | Broad three-archetype Pokémon ex blocker now that the shared Supporter pair is closed. |
...
1. Take the remaining highest-share three-archetype blockers `TWM-112`, `ASC-216`, and `ASC-121` next.
2. After that, continue the remaining three-archetype metadata-missing slice with `TWM-053`, `POR-087`, `POR-072`, and `TEF-154` before dropping into lower-archetype blockers.
